...
struct Reader {
 public:
  ...
  void run() {
    while (1) {
      ...
      uint64_t shard;
      {
        std::lock_guard<boost::fibers::mutex> guard(queue_mutex_);
 ...
 private:
  ...
  sem_t sem_;
  boost::fibers::mutex queue_mutex_;
...
struct StatsThread {
  ...
  void run() {
    ...
    while (!done_.load()) {
      {
        std::unique_lock<boost::fibers::mutex> lock(cvm_);
  ...
  ~StatsThread() {
    {
      std::lock_guard<boost::fibers::mutex> guard(cvm_);
      done_.store(true);
    }
    cv_.notify_all();
    thread_.join();
